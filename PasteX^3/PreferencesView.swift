...
struct GeneralPane: View {
    ...
    @AppStorage("autoCleanup")     var autoCleanup     = true
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                VStack(alignment: .leading, spacing: 8) {
                    Toggle("Open at login", isOn: $launchAtLoginManager.isEnabled)
                        .onChange(of: launchAtLoginManager.isEnabled) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Toggle("Auto-cleanup old clips", isOn: $autoCleanup)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Export your clipboard history to a file for backup or transfer to another device.")
                        .font(.caption)
        ...
        .fileExporter(
            ...
            document: ClipboardExportDocument(clips: clips),
            contentType: .json,
            defaultFilename: "PasteX_Backup_\(Date().formatted(.dateTime.year().month().day()))"
        ) { result in
            switch result {
        ...
        .fileImporter(
            ...
            case .success(let urls):
                if let url = urls.first {
                    let success = ClipboardExporter.importFromFile(url: url, context: modelContext)
                    if success {
